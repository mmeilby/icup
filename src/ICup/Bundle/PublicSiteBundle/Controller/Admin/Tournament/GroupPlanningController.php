...
class GroupPlanningController extends Controller
{
    ...
    /**
     * List the clubs by groups assigned in the category
     * @Route("/edit/list/grps/{categoryid}/clss", name="_host_list_playoff")
     * @Template("ICupPublicSiteBundle:Host:listcategory_class.html.twig")
     */
    public function listPlayoffByCategoryAction($categoryid)
    {
        /* @var $utilService Util */
        $utilService = $this->get('util');
        /* @var $user User */
        ...
        $groups = $this->get('logic')->listGroups($categoryid, 1);
        $teamsUnassigned = $this->get('logic')->listTeamsEnrolledUnassigned($categoryid, 1);
        $groupList = array();
        $selectedGroup = null;
        $preferredGroup = $this->getSelectedGroup();
        foreach ($groups as $group) {
    ...
    /**
     * List the clubs by groups assigned in the category
     * @Route("/edit/list/grps/{categoryid}/fnls", name="_host_list_finals")
     * @Template("ICupPublicSiteBundle:Host:listcategory_finals.html.twig")
     */
    public function listFinalsByCategoryAction($categoryid)
    {
        /* @var $utilService Util */
        $utilService = $this->get('util');
        /* @var $user User */
        ...
        $groups = $this->get('logic')->listGroups($categoryid, 8);
        $teamsUnassigned = $this->get('logic')->listTeamsEnrolledUnassigned($categoryid, 8);
        $groupList = array();
        $selectedGroup = null;
        $preferredGroup = $this->getSelectedGroup();
        foreach ($groups as $group) {
