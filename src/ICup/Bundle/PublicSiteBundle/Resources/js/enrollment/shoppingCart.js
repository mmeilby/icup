...
angular.module('shopModule', [])
    .factory('Store', function () {
        ...
        var myCart = new shoppingCart("iCupStore");
        return {
            store: myStore,
            cart: myCart
        };
        ...
        try {
            ...
            for (var i = 0; i < items.length; i++) {
                ...
                if (item.sku != null && item.name != null && item.price != null && item.quantity != null) {
                    item = new cartItem(item.sku, item.name, item.price, item.deposit, item.quantity);
...
shoppingCart.prototype.addItem = function (sku, name, price, deposit, quantity) {
    ...
    if (quantity != 0) {
        ...
        // new item, add now
        if (!found) {
            var item = new cartItem(sku, name, price, deposit, quantity);
            this.items.push(item);
        }
...
// get the total deposit for all items currently in the cart
shoppingCart.prototype.getTotalDeposit = function (sku) {
    var total = 0;
    for (var i = 0; i < this.items.length; i++) {
        var item = this.items[i];
        if (sku == null || item.sku == sku) {
            total += this.toNumber(item.quantity * item.deposit);
        }
    }
    return total;
}
...
// items in the cart
//
function cartItem(sku, name, price, deposit, quantity) {
    this.sku = sku;
    this.name = name;
    this.price = price * 1;
    this.deposit = deposit * 1;
